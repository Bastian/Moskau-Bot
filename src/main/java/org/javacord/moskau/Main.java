...
public class Main {
        ...
        api.addMessageCreateListener(event -> {
            ...
            voiceChannel.connect()
                    .thenAcceptAsync(connection -> {
                        ...
                        connection.queue(YouTubeAudioSource.of(api, "https://youtu.be/NvS351QKFV4").join());
                        connection.addAudioSourceFinishedListener(e -> connection.close());
                    })
                    .exceptionally(throwable -> {
                        event.getChannel().sendMessage("Failed to play song!");
                        logger.warn("Failed to play song!", throwable);
                        return null;
                    });
        });
    }
    ...
    private static void sendMoskauMessage(Messageable receiver) {
        MessageBuilder messageBuilder = new MessageBuilder()
                .setContent("MOSKAU MOSKAU, RUSSLAND IST EIN SCHÖNES LAND! :musical_note: :musical_note: :musical_note:");
